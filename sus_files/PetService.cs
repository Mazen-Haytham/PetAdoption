...
namespace backend.Pets.Services
{
    ...
    {
        /// <summary>Returned when the JSON body binds but every field is null/omitted.</summary>
        public const string NoPatchFieldsMessage = PetListingPatchMessages.NoPatchFields;
        ...
        // ── GET MY POSTS ────────────────────────────
        /// <summary>
        /// Not cached: shelter owners create/update/delete often; stale cache caused deleted rows to reappear.
        /// </summary>
        ...
        // ── CREATE ──────────────────────────────────
        public async Task<(Pet Pet, int PetPostId)> CreatePetAsync(CreatePetDto dto, int ownerId)
        {
            using var transaction = await _petRepository.BeginTransactionAsync();
            var savedImagePaths = new List<string>();
            int createdPetPostId = 0;
            ...
            try
            {
                ...
                await InvalidatePetCachesAsync(ownerId, createdPetPostId);
        ...
        {
            var (ok, message) = await _petRepository.TryPatchPetPostAsync(petPostId, ownerId, dto);

            if (!ok)
                return (false, message, null);

            await InvalidatePetCachesAsync(ownerId, petPostId);

            return (true, message, new { petPostId });
        }
        ...
        public async Task<(bool Success, string Message)> DeletePetAsync(int petPostId, int ownerId)
        {
            ...
            try
            {
                ...
                if (petPost.Adoptions != null)
                    return (false, "This listing has an adoption on file and cannot be deleted.");
        ...
        private async Task InvalidatePetCachesAsync(int ownerId, int? petPostId)
        {
            _memory.Remove(AllPetPostsCacheKey);
            _memory.Remove(OwnerPetPostsCacheKey(ownerId));
            await _redis.RemoveAsync(AllPetPostsCacheKey);
            await _redis.RemoveAsync(OwnerPetPostsCacheKey(ownerId));

            if (!petPostId.HasValue) return;

            var id = petPostId.Value;
            var singleKey = SinglePetPostCacheKey(id);
            var detailKey = $"petPosts:detail:{id}";
            _memory.Remove(singleKey);
            _memory.Remove(detailKey);
            await _redis.RemoveAsync(singleKey);
            await _redis.RemoveAsync(detailKey);
        }
    }
}